...
export const getUsers = createAsyncThunk('getUsers', async (user: number, thunkAPI) => {
    try {
...
export const filterBusinesses = createAsyncThunk('admin/filterBusinesses', async (filterValue, thunkAPI) => {
    try {
        return filterValue;
...
export const companyFaq = createAsyncThunk('companies / faqs', async (_, thunkAPI) => {
    try {
        return await businessService.companyFaq();
    } catch (error: any) {
        const message = (error.response && error.response.data && error.response.data.message) || error.message || error.toString;
...
export const updateTicketStatus = createAsyncThunk('ticket update', async (data: any, thunkAPI) => {
    try {
        console.log('jknbkj');
...
export const getCompanyEmailForwarderConversation = createAsyncThunk('companies/emailForwarderConversation', async (provider: string, thunkAPI) => {
    try {
...
// });
// getTicketSummary;
export const updateCompanyFaq = createAsyncThunk('update/companies / faqs', async (newKnowledgeBase: any, thunkAPI) => {
    try {
...
export const addCompanyFaq = createAsyncThunk('add /companies / faqs', async (knowledgeBase: { knowledgeBase: any[] }, thunkAPI) => {
    try {
...
export const deleteCompanyFaq = createAsyncThunk('delete each knowledgebase', async (knowledgeBaseId: string, thunkAPI) => {
    try {
...
export const sendTicketResponse = createAsyncThunk('sendTicketRedsponse', async (data: any, thunkAPI) => {
    try {
...
export const addDepartment = createAsyncThunk('addDepartment', async (data: any, thunkAPI) => {
    try {
...
export const getDepartments = createAsyncThunk('getDepartment', async (data, thunkAPI) => {
    try {
...
export const getConversations = createAsyncThunk('get conversation', async (thunkAPI) => {
    try {
...
export const getEachConversation = createAsyncThunk('get each conversation', async (data: any, thunkAPI) => {
    try {
...
export const sendMessage = createAsyncThunk('send message', async (data: any, thunkAPI) => {
    try {
...
export const companyProfile = createAsyncThunk('company profile', async (thunkAPI) => {
    try {
...
export const updateCompanyAutoResponderStatus = createAsyncThunk('update company auto response', async (status: boolean, thunkAPI) => {
    try {
...
export const updateCompanyDetails = createAsyncThunk('update company profile', async (data: any, thunkAPI) => {
    try {
...
export const updateChatStatus = createAsyncThunk('update user chat status', async (customerId: string, thunkAPI) => {
    try {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(filterBusinesses.fulfilled, (state, action: any) => {
                ...
                state.companies = action.payload.allCompanies.filter((company: { status: string }) => {
            ...
            .addCase(companyFaq.fulfilled, (state, action: any) => {
                state.isLoading = false;
                console.log(action.payload.data);
            ...
            .addCase(updateCompanyFaq.fulfilled, (state, action: any) => {
                ...
                const index = state.faqs.findIndex((faq: any) => faq?.knowledgebase_id === response?.knowledgebase_id);
                state.faqs = [
            ...
            .addCase(getCompanyEmailForwarderConversation.fulfilled, (state, action: any) => {
            ...
            .addCase(updateTicketStatus.fulfilled, (state, action: any) => {
                ...
                let hashMapIds = response.updatedRecordId.reduce((store: any, id: number) => {
                    store[`${id}`] = id;
                    return store;
                }, {});
                state.emailForwarderMessages = [...state.emailForwarderMessages].map((conversation) => {
            ...
            .addCase(deleteCompanyFaq.fulfilled, (state, action: any) => {
                state.faqs = [...state.faqs].filter((faq: any) => faq?.knowledgebase_id != action.payload?.data?.knowledgeBaseId);
